...
- `/show/<tmdbId>` — **the show page** (signed in; the destination of a search result tap). Server-rendered for a TV show: backdrop/poster, title, year, and overview, followed by a **"Seasons"** list. Season 0 / specials, if present, sort to the end. Each season is a button that **expands to reveal its episodes** (episode number + name), fetched same-origin from `/api/tmdb/season?show=<id>&season=<n>` — the browser must never call TMDB directly here either. Each episode row has a compact **"Log"** button. Tapping it opens the log dialog with the show, season, and episode **already chosen from context**: the dialog has **no season/episode picker** — it shows what's being logged as static text (title, then `S<n>E<m> · <episode name>`) plus watched-date, tags, note, and rewatch fields. Submitting writes records to the test account's PDS — logging during verification is safe and expected. Guard: a season with no episodes shows an empty state, not an error; season data loads must not 404.
